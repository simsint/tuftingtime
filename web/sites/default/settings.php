...
$databases['default']['default'] = [
  'database' => getenv('DB_DATABASE') ?: 'tuftingtime_site',
 ...
 * @endcode
 */
$settings['hash_salt'] = getenv('DRUPAL_HASH_SALT') ?: 'f2e660691510d776856d0238713ef5d161c622b5fb470a0e5428ccdce8e8cae8';
...
if ($prod_domain = getenv('APP_DOMAIN')) {
  $settings['trusted_host_patterns'][] = '^' . preg_quote($prod_domain) . '$';
  $settings['trusted_host_patterns'][] = '^www\.' . preg_quote($prod_domain) . '$';
}
...
if ($app_env === 'production') {
  ...
  // Set error logging to log only
  $config['system.logging']['error_level'] = 'hide';

  // Set trusted host patterns from environment
...
if ($app_env === 'local' || $app_env === 'development') {
  ...
  if (isset($app_root) && isset($site_path) && file_exists($app_root . '/' . $site_path . '/development.services.yml')) {
